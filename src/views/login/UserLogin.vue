<template>
  <div class="loginBody">
    <div class="loginForm">
      ...
      <router-link style="float: left" class="linkFont" to="/inspectUp">
...
export default {
  ...
  methods: {
    chkLogin(){
      console.log("처음 생성213123123");
      console.log(this.$store.getters.getUserLogin);
      console.log(this.$store.getters.getUserLogin.status);
      if(this.$store.getters.getUserLogin.status === 200 && this.$store.getters.getUserLogin.data.body.result === 'success'){
      console.log("처음 생성");
      this.roturInit();
    }
    },
